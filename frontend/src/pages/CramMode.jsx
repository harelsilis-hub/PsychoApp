...
import { ArrowRight, Flame, XCircle, Trophy } from 'lucide-react';
...
// ─── Results screen ────────────────────────────────────────────────────────────
const CramResult = ({ score, skipped, total, onRetry, onBack }) => {
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="text-gray-500 mb-6">
          ענית נכון על{' '}
          <span className="font-bold text-amber-500">{score} / {total}</span>{' '}
          ({pct}%)
        </p>
...
const CramMode = () => {
  ...
  const loadQuiz = async () => {
    ...
    try {
      const data  = await reviewAPI.getCramWords(20, language);
      ...
      setError('שגיאה בטעינת מצב החרישה. נסה שוב.');
    } finally {
      setLoading(false);
    }
  };
  ...
  }); // run on every render to capture latest handleSelect closures
  ...
  const handleSelect = (opt) => {
    ...
    reviewAPI.submitCram(currentQ.word_id, opt.isCorrect ? 4 : 1)
      .then((result) => {
      ...
      .catch((err) => console.error('Cram submit failed:', err));
  };
  ...
  const handleDontKnow = () => {
    ...
    setSkipped((s) => s + 1);
    reviewAPI.submitCram(currentQ.word_id, 0)
      .catch((err) => console.error('Cram submit failed:', err));
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">טוען מצב חרישה...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">אין מילים לחרישה</h2>
          <p className="text-gray-500 mb-6 text-sm leading-relaxed">
            צריך לסיים לפחות יחידה אחת בחזרה היומית כדי להשתמש במצב החרישה.
          </p>
          <button
            onClick={() => navigate('/')}
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-white/80 backdrop-blur border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <span className="font-semibold text-gray-800 flex-1">מצב חרישה</span>
          <span className="text-sm text-gray-500 font-medium">
            {qIndex + 1} / {questions.length}
          </span>
          <SoundToggle />
        </div>
        {/* Amber progress bar to distinguish from daily review */}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                {currentQ.options.map((opt, i) => {
                  let style = 'bg-white border-2 border-gray-200 text-gray-800 hover:border-amber-400 hover:bg-amber-50';
                  if (isAnswered) {
                    if (opt.isCorrect)
              ...
              {/* Cram mode note — no SM-2 impact */}
              {isAnswered && (
                <motion.p
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="text-center text-xs text-amber-500 mb-2 font-medium"
                >
                  🔥 מצב חרישה — תרגול בלי השפעה על לוח הזמנים של האלגוריתם
                </motion.p>
              )}
